...
Legacy compatibility alias for `investigate-and-implement`. Prefer the newer skill name for fresh docs and installs, but keep this alias around so older `skills add` commands and tree URLs keep working.
...
- Finish by opening or updating the PR, and when the PR is later merged, clean up the worktree and summarize the outcome.
